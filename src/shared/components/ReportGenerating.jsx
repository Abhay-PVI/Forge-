...
export default function ReportGenerating({ fname, steps, onDone, speed = 500 }) {
  ...
  return (
    <div style={{ flex: 1, display: 'grid', placeItems: 'center', background: 'var(--bg)' }}>
      <div style={{ width: 340, textAlign: 'center' }} className="fade-up">
        ...
        <div className="mono" style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 24 }}>{fname}</div>
          ...
          {steps.map((s, idx) => {
            ...
            return (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 11, opacity: done || active ? 1 : 0.4, transition: 'opacity .3s' }}>
                {done ? (
                  <span style={{ width: 20, height: 20, borderRadius: '50%', background: 'var(--accent)', color: '#fff', display: 'grid', placeItems: 'center', flex: 'none' }}>
                    <Icon name="check" size={12} stroke={3} />
                  </span>
                ) : active ? (
                  <Icon name="settings" size={20} className="spin" style={{ color: 'var(--accent)', flex: 'none' }} />
                ) : (
