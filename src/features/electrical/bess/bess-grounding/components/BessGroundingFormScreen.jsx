...
function overallStatus(values, files) {
  const total = GROUNDING_ANALYSIS_TABS.reduce((sum, tab) => sum + tabRequiredKeys(tab).length, 0);
  let done = 0;

  GROUNDING_ANALYSIS_TABS.forEach((tab) => {
...
function docNumber(values) {
  return `${values?.documentNo || "PVI-BESS-GRN-001"}-${values?.revision || "A"}`;
}
...
function ChecklistRail({ values, files, step, setStep }) {
  return (
    <div className="card" style={{ padding: 14 }}>
      ...
      {GROUNDING_ANALYSIS_TABS.map((tab, index) => {
...
export default function BessGroundingFormScreen({ report, vertical, sub, values, setValue, files, setFile, calc, layout, showCalc, onGenerate, onSaveDraft, onClearAll }) {
  ...
  const loadLastEntry = async () => {
    try {
      setBanner(null);
      const res = await fetchLastReportApi("grounding");
      if (res.success && res.data) {
        ...
        setBanner({
          type: "success",
          text: `Loaded details from last grounding entry (Source ID: ${metadata.id}).`
        });
      } else {
        setBanner({
          type: "warning",
      ...
      console.error("Error loading last BESS grounding entry:", err);
      setBanner({
        type: "error",
  ...
  const tab = GROUNDING_ANALYSIS_TABS[step];
  const isLast = step === GROUNDING_ANALYSIS_TABS.length - 1;
